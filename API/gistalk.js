...
router.get('/', async (req, res) => {
  ...
  if (!uid) return res.status(400).json({ status: 400, message: 'Parameter ?uid= is required.' });
  ...
  try {
    ...
    if (!data.playerInfo) {
      return res.status(404).json({ status: 404, message: 'Data not found for that UID.' });
    }
    ...
    res.status(500).json({ status: 500, message: 'Failed to fetch data.', error: err.message });
  }
});

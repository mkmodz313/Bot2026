...
router.get('/', async (req, res) => {
  ...
  if (!link) return res.status(400).json({ status: false, message: 'URL cannot be empty' });
  ...
  try {
    ...
    res.status(500).json({ status: false, message: 'Failed to get download link', error: err.message });
  }
});
